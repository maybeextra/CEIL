...
class Tester(Base_trainer):
    ...
    def draw(self, args, test_mode, mode):
        ...
        query_feat, query_label, query_cam, query_img = self.extract_features(query_loader, test_mode[0], False, True)

        gall_loader = creat_test_data(args, 1, mode=mode, kind='image')
        gall_feat, gall_label, gall_cam, gall_img = self.extract_features(gall_loader, test_mode[1], False, True)
    ...
    def test(self, args, test_mode, mode):
        ...
        for trial in range(1, num_iter + 1):
            ...
            if args.dataset == 'SYSU':
                ...
                query_loader = creat_test_data(args, trial, mode=mode, kind='query')
                query_feat, query_label, query_cam = self.extract_features(query_loader, test_mode[0], False)
                _eval = eval_regdb
            elif args.dataset == 'LLCM':
                _eval = eval_llcm
