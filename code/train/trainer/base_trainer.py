...
class Base_trainer:
    ...
    def extract_features(self, data_loader, modal, flag = True):
    ...
    def valid(self, args, test_mode, mode):
        ...
        for trial in range(1, num_iter+1):
            gall_loader = creat_test_data_gall(args, trial=trial, mode=mode)
            ...
            if args.dataset == 'SYSU':
                ...
                query_loader = creat_test_data_query(args, trial=trial, mode=mode)
                query_feat, query_label, query_cam = self.extract_features(query_loader, test_mode[0], False)
                _eval = eval_regdb
