...
def creat_test_data_query(args, trial=1, mode=None):
    if args.dataset == 'SYSU':
        img, label, cam = process_images(data_path=args.data_dir, img_w=args.img_w, img_h=args.img_h, mode=mode, dataset='SYSU',data_type='query')
        transform = transform_extract_SYSU
    elif args.dataset == 'RegDB':
        img, label, cam = process_test_regdb(data_path=args.data_dir, trial=trial, mode=mode, kind='query')
        transform = transform_extract_RegDB
    elif args.dataset == 'LLCM':
        img, label, cam = process_images(data_path=args.data_dir, img_w=args.img_w, img_h=args.img_h, mode=mode, dataset='LLCM',data_type='query')
        transform = transform_extract_SYSU
    else:
...
def creat_test_data_gall(args, trial=1, mode=None):
    if args.dataset == 'SYSU':
        img, label, cam = process_images(data_path=args.data_dir, img_w=args.img_w, img_h=args.img_h, mode=mode, dataset='SYSU',data_type='gallery', trial=trial)
        transform = transform_extract_SYSU
    elif args.dataset == 'RegDB':
        img, label, cam = process_test_regdb(data_path=args.data_dir, trial=trial, mode=mode, kind='gall')
        transform = transform_extract_RegDB
    elif args.dataset == 'LLCM':
        img, label, cam = process_images(data_path=args.data_dir, img_w=args.img_w, img_h=args.img_h, mode=mode, dataset='LLCM',data_type='gallery', trial=trial)
        transform = transform_extract_LLCM
    else:
        raise ValueError('Please input correct dataset!!')
