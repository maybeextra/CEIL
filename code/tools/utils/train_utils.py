...
def extract_features(data_type, args, trainer, cluster):
    transform_extract = trans.creat_extract(args.dataset)
    ...
    features, labels, cameras = trainer.extract_features(cluster_loader, modal=data_type)
...
def creat_dic(pseudo_labels, proxy_labels, unique_cameras, cams):
    ...
    cam2proxy = [[] for _ in range(num_cameras)]
    for cam in unique_cameras:
        index = torch.where(cams == cam)[0]
        cam2instance[cam] = index
        cam2proxy[cam] = proxy_labels[index].unique().cuda()
    ...
    return label2proxy, cam2proxy, proxy2label, proxy2cam, label2instance, proxy2instance, cam2instance, instance2label, instance2proxy, instance2cam
